...
//		$this->session->userdata['groups'] = $this->permission_test_m->my_groups($this->session->userdata['logged_in']['userid']);
	    ...
	    foreach($this->session->userdata['groups'] as $this_group)
		    $this->data['my_groups'][$this_group['group_id']] = $this->permission_test_m->get_group_data($this_group['group_id']);
...
//	    echo 'view_user() not working yet';
	    $this->data['user'] = $this->permission_test_m->get_user($userid);
 ...
 */
    ...
    {
	    $from_form = false;
//	// if no parameters are passed in (comes from a form)
//	    if($entityid=='' && $groupid=='')
//		{
//			if($this->input->post())
//			{
////die ('from form');
//				$from_form = true;
//				$data['entityid'] = $this->input->post('userid');
...
//		}
if($this->permission_test_m->in_group($entityid, $groupid))
	   die( 'You\'re already in this group' );
//pass values to the model		
		if($this->permission_test_m->join_group($data))
		{
//update the session array
$this->session->set_userdata('groups', $this->permission_test_m->my_groups($entityid));	
